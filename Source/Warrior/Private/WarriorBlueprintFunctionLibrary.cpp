...
UPawnCombatComponent* UWarriorBlueprintFunctionLibrary::NativeGetPawnCombatComponentFromActor(AActor* InActor)
{
	...
	{
		return PawnCombatInterface->GetPawnCombatCombatComponent();
	}
	return nullptr;
}
	...
	float& OutAngleDifference)
{
	...
	// Determine the hit react direction based on the angle difference
...
bool UWarriorBlueprintFunctionLibrary::IsValidBlock(AActor* InAttacker, AActor* InDefender)
{
	...
	//const FString DebugString = FString::Printf(TEXT("Dot Result: %f %s"), DotResult, DotResult<-0.1f ? TEXT("Block is Valid") : TEXT("Block is Invalid"));
